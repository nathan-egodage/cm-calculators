...
const AusFteGpCalculator = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-6xl">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">AUS FTE GP Calculator</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">Calculate gross profit for Australian full-time employees</p>
      </div>
      ...
      <Card className="mb-6 border border-green-100 dark:border-green-900">
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant={calculationMode === 'salaryPackage' ? 'primary' : 'outline'}
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-4">
              {/* Salary Package */}
              <FormGroup>
                <FormLabel htmlFor="salary-package">
                  AUD$ Salary Package (Including Super)
                  {calculationMode === 'salaryPackage' && 
                ...
                <FormInput
                  ...
                  step="0.01"
                  value={salaryPackage}
                  onChange={(e) => setSalaryPackage(parseFloat(e.target.value) || 0)}
            ...
            <table className="w-full">
              <tbody>
                <tr className={`border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                  <td className="px-4 py-3 text-sm">Salary Package (Including Super)</td>
                  <td className="px-4 py-3 text-right font-medium">{formatCurrency(salaryPackage)}</td>
                </tr>
                <tr className={`border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
